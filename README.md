...
- **لا تستورد البيانات مباشرة من `@/services/mock`** داخل `pages/` أو `components/` أو `context/` أو `hooks/` — الوصول للبيانات يمرّ حصرًا عبر طبقة الخدمات النظيفة (`services/<entity>`) وعبر الـ hooks (`useClients`, `useProducts`, `useReporting` …). تفرض قاعدة ESLint `no-restricted-imports` هذا الحظر وترفض البناء عند مخالفتها.
...
**سادسًا — احترم قاعدة الاعتمادية (Dependency Rule):** يعتمد المشروع نمط **Clean Architecture** لطبقة البيانات. الاتجاه الإلزامي للاعتماديات هو: `الواجهة (pages/components) → الـ hook → الخدمة (Service) → الواجهة المجرّدة (IRepository) → التنفيذ الفعلي (MockRepository)`. لا تعرف الواجهة شيئًا عن مصدر البيانات، ولا تستورد منه مباشرة. أضف خدمة جديدة بنفس النمط الرباعي: `IXRepository.ts` (الواجهة) ← `MockXRepository.ts` (التنفيذ) ← `XService.ts` (منطق العمل) ← `index.ts` (التصدير)، ثم سجّلها في `src/context/data-services-context.tsx` (نقطة التكوين الوحيدة المسموح لها بلمس التنفيذات)، ثم اكشفها عبر hook. عند توفّر backend حقيقي، يُستبدل `new MockXRepository()` بـ `new FirestoreXRepository()` في مكان واحد فقط دون تغيير أي صفحة. راجع [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).

**سابعًا — قبل الـ commit:** شغّل `npm run lint` و`npm run build` للتأكد من عدم وجود أخطاء. يجب أن يمرّا بـ 0 أخطاء.
