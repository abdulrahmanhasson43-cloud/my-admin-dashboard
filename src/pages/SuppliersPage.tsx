...
export default function SuppliersPage() {
  ...
  const { suppliers, createSupplier, updateSupplier, deleteSupplier, restoreSupplier } = useSuppliers();
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      // وضع التعديل — تحديث المورد الحالي في مكانه.
  ...
  /** يفتح النموذج معبّأً ببيانات المورد الحالية (وضع التعديل). */
  ...
  /** يحذف المورد فورًا مع إتاحة التراجع عبر إشعار Toast. */
  const handleDelete = async (supplier: Supplier) => {
    await deleteSupplier(supplier.id);
    toast.success(`تم حذف المورد ${supplier.name}`, {
      action: { label: 'تراجع', onClick: () => restoreSupplier(supplier) },
    });
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="card-vuno p-5">
          <h3 className="font-bold text-[var(--vuno-text)] mb-4">{editingId ? 'تعديل المورد' : 'إضافة مورد جديد'}</h3>
          ...
          <div className="flex gap-3 mt-4 justify-end">
            <button onClick={() => { setShowForm(false); setEditingId(null); }} className="px-5 py-2.5 rounded-xl border border-[var(--vuno-border)] text-[var(--vuno-text-secondary)] hover:bg-[var(--vuno-bg)] transition-colors">إلغاء</button>
            <button onClick={handleSave} className="px-5 py-2.5 rounded-full text-white font-medium hover:opacity-90 transition-opacity" style={{ background: 'var(--vuno-primary)' }}>{editingId ? 'حفظ التعديلات' : 'حفظ'}</button>
          </div>
        </motion.div>
      )}
        ...
        {filtered.map((s, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-[var(--vuno-border-light)]">
              ...
              <button onClick={() => handleEdit(s)} className="flex-1 h-10 rounded-xl flex items-center justify-center gap-2 text-[var(--vuno-primary)] border border-[var(--vuno-border)] hover:bg-[var(--vuno-bg)] transition-colors text-sm font-medium" aria-label="تعديل">
                <EditIcon size={14} />
                تعديل
              </button>
              <button onClick={() => handleDelete(s)} className="w-10 h-10 rounded-xl flex items-center justify-center text-red-500 border border-red-200 hover:bg-red-50 transition-colors" aria-label="حذف">
                <TrashIcon size={14} />
              </button>
            </div>
          </motion.div>
