...
function ClientStatusBadge({ status, className = '' }: { status: Client['status']; className?: string }) {
  return (
    <span className={`inline-block rounded-full font-medium ${status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500'} ${className}`}>
...
function ClientSidePanel({
  ...
  const { getLoyaltySummary, getPointsEarned, getPointsRedeemed } = useLoyalty();
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <ClientStatusBadge status={client.status} className="mt-1 px-2.5 py-0.5 text-[11px]" />
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
              ...
              >
                <span className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: '#007AFF1A' }}>
                  <PhoneIcon size={20} style={{ color: '#007AFF' }} />
                </span>
                <div className="flex-1">
              ...
              {client.totalPurchases > 20000 && (
                <div className="rounded-[14px] p-4 flex items-center gap-3" style={{ background: '#FFD7001A', border: '1px solid #FFD70044' }}>
                  <StarIcon size={24} style={{ color: '#FFD700' }} className="flex-shrink-0" />
                  <div>
          ...
          {tab === 'loyalty' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <LoyaltySystem
                ...
                clientId={client.id}
                earnedHistory={getPointsEarned(client.id)}
                redeemedHistory={getPointsRedeemed(client.id)}
              />
            </motion.div>
          )}
        </div>
...
export default function ClientsPage() {
  ...
  const { clients, createClient, getClientActivities } = useClients();
  ...
  const clientActivitiesFor = (clientId: string) => getClientActivities(clientId);
        ...
        {filtered.map((client, i) => (
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              ...
              <ClientStatusBadge status={client.status} className="px-2.5 py-1 text-xs flex-shrink-0" />
            </div>
