...
import type { SalesPoint } from '@/services/reporting';
...
const paymentMethodData = [
  { name: 'كاش', value: 45, color: 'var(--vuno-primary)' },
  { name: 'بطاقة', value: 30, color: 'var(--vuno-success)' },
  { name: 'محفظة', value: 18, color: 'var(--vuno-warning)' },
  { name: 'إنستاباي', value: 7, color: '#AF52DE' },
];

// الربح حسب الشهر
const buildProfitData = (sales: SalesPoint[]) => sales.map(d => ({
  name: d.name,
  revenue: d.sales,
...
export default function ReportsPage() {
  ...
  // Reporting data comes from the clean Reporting service (never the mock layer).
  const { getSalesData, getTopProducts } = useReporting();
  const salesData = useMemo(() => getSalesData(), [getSalesData]);
  const topProducts = useMemo(() => getTopProducts(), [getTopProducts]);
          ...
          <AreaChart data={periodData} margin={{ top: 6, right: 8, left: 4, bottom: 0 }}>
            <defs>
              <linearGradient id="salesGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="var(--vuno-primary)" stopOpacity={0.25} />
                <stop offset="100%" stopColor="var(--vuno-primary)" stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="0" stroke="var(--vuno-border-light)" vertical={false} />
            <XAxis dataKey="name" tick={{ fontSize: 11, fill: 'var(--vuno-text-muted)' }} axisLine={false} tickLine={false} />
            <YAxis
              tick={{ fontSize: 11, fill: 'var(--vuno-text-muted)' }}
              axisLine={false}
              tickLine={false}
              width={50}
              tickFormatter={v => `${(v / 1000).toFixed(0)}k`}
            />
            <Tooltip content={<ChartTooltip />} cursor={{ stroke: 'var(--vuno-primary)', strokeWidth: 1, strokeDasharray: '4 4' }} />
            <Area
              type="monotone"
              dataKey={dataKey}
              name="المبيعات"
              stroke="var(--vuno-primary)"
              strokeWidth={2.5}
              fill="url(#salesGradient)"
              dot={{ fill: 'var(--vuno-primary)', r: 3 }}
              activeDot={{ r: 5 }}
            />
          </AreaChart>
        </ResponsiveContainer>
        ...
        <ChartCard title="الإيراد حسب الفئة" subtitle="توزيع المبيعات على الفئات" delay={0.25}>
          <ResponsiveContainer width="100%" height={240} minWidth={200}>
            <BarChart data={categoryRevenueData} margin={{ top: 6, right: 8, left: 4, bottom: 0 }}>
              <CartesianGrid strokeDasharray="0" stroke="var(--vuno-border-light)" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 10, fill: 'var(--vuno-text-muted)' }} axisLine={false} tickLine={false} />
              <YAxis
                tick={{ fontSize: 11, fill: 'var(--vuno-text-muted)' }}
                axisLine={false}
                tickLine={false}
              ...
              <Bar dataKey="value" name="الإيراد" radius={[6, 6, 0, 0]} fill="var(--vuno-primary)" barSize={32} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
          ...
          <ResponsiveContainer width="100%" height={240} minWidth={200}>
            <PieChart>
              ...
              <Legend
                ...
                iconType="circle"
                wrapperStyle={{ fontSize: 12, color: 'var(--vuno-text-secondary)' }}
              />
            </PieChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ResponsiveContainer width="100%" height={260} minWidth={200}>
          <LineChart data={profitData} margin={{ top: 6, right: 8, left: 4, bottom: 0 }}>
            ...
            <YAxis
              ...
              tickFormatter={v => `${(v / 1000).toFixed(0)}k`}
            />
            <Tooltip content={<ChartTooltip />} cursor={{ stroke: 'var(--vuno-text-muted)', strokeWidth: 1, strokeDasharray: '4 4' }} />
            <Legend
              verticalAlign="top"
              iconType="circle"
              wrapperStyle={{ fontSize: 12, color: 'var(--vuno-text-secondary)', paddingBottom: 12 }}
            />
            <Line
              type="monotone"
              dataKey="revenue"
            ...
            <Line
              ...
              dataKey="profit"
              name="الربح"
              stroke="var(--vuno-success)"
              strokeWidth={2.5}
              dot={{ fill: 'var(--vuno-success)', r: 3 }}
              activeDot={{ r: 5 }}
            />
          </LineChart>
        </ResponsiveContainer>
