...
export default function BranchesPage() {
  ...
  const saveBranch = () => {
    ...
    if (editingBranch) {
      updateBranch(editingBranch.id, { name: branchName.trim(), address: branchAddress.trim() });
      toast.success(`تم تحديث فرع ${branchName.trim()}`);
    } else {
      addBranch({
      ...
      toast.success(`تمت إضافة فرع ${branchName.trim()}`);
    }
    setShowForm(false);
    setEditingBranch(null);
  };
        ...
        <>
          ...
          {activeBranch && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-bold text-[var(--vuno-text)] truncate">{activeBranch.name}</h3>
                <p className="text-xs text-[var(--vuno-text-secondary)] truncate">{activeBranch.address}</p>
              </div>
          ...
          {showForm && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="card-vuno p-5 overflow-hidden">
              <h3 className="font-bold text-[var(--vuno-text)] mb-4">{editingBranch ? 'تعديل الفرع' : 'إضافة فرع جديد'}</h3>
              ...
              <div className="flex gap-3 mt-4 justify-end">
                <button onClick={() => { setShowForm(false); setEditingBranch(null); }} className="px-5 py-2.5 rounded-xl border border-[var(--vuno-border)] text-[var(--vuno-text-secondary)] hover:bg-[var(--vuno-bg)] transition-colors">إلغاء</button>
                <button onClick={saveBranch} className="px-5 py-2.5 rounded-xl text-white font-medium hover:opacity-90 transition-opacity" style={{ background: 'var(--vuno-primary)' }}>{editingBranch ? 'حفظ التعديلات' : 'حفظ'}</button>
              </div>
            </motion.div>
          )}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {filtered.map((branch, i) => (
              <motion.div key={branch.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.1, 0.3) }} className={`card-vuno p-6 hover:shadow-md transition-all ${activeBranchId === branch.id ? 'ring-2 ring-[var(--vuno-primary)] border-[var(--vuno-primary)]' : ''}`}>
                ...
                <div className="grid grid-cols-3 gap-4 pt-4 border-t border-[var(--vuno-border-light)]">
                  ...
                  <div className="flex items-center justify-center gap-1.5">
                    ...
                    <button onClick={() => openBranchForm(branch)} aria-label={`تعديل ${branch.name}`} className="p-2 rounded-lg hover:bg-[var(--vuno-bg)] text-[var(--vuno-primary)] transition-colors"><EditIcon size={16} /></button>
                    <button onClick={() => removeBranch(branch)} aria-label={`حذف ${branch.name}`} className="p-2 rounded-lg hover:bg-red-50 text-red-500 transition-colors"><TrashIcon size={16} /></button>
                  </div>
                </div>
                {activeBranchId !== branch.id && (
                  <button
                    onClick={() => { setActiveBranchId(branch.id); toast.success(`تم التبديل إلى فرع ${branch.name}`); }}
