...
function StockStatusBadge({ isLow, className = '' }: { isLow: boolean; className?: string }) {
  return (
    <span className={`rounded-lg text-xs font-medium ${isLow ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'} ${className}`}>
      {isLow ? 'منخفض' : 'متوفر'}
    </span>
  );
}
...
export default function InventoryPage() {
  ...
  const handleQuickTransfer = async (product: Product, qty = 1) => {
    ...
    try {
      if (needsConfirmation) {
        await requestTransfer(product.id, moveQty);
        toast.success(`تم إرسال طلب نقل ${moveQty} قطعة — بانتظار التأكيد`);
      } else {
        await transferToStore(product.id, moveQty);
  ...
  const handleDropToStore = () => {
    if (dragProduct) {
      void handleQuickTransfer(dragProduct, 1);
      setDragProduct(null);
    }
  };
  ...
  const handleTransferSubmit = async () => {
    ...
    if (qty > 0 && qty <= transferProduct.warehouseStock) {
      try {
        if (needsConfirmation) {
          await requestTransfer(transferProduct.id, qty);
        } else {
          await transferToStore(transferProduct.id, qty);
        }
      } catch {
        // The context has already surfaced the error (banner + toast).
      }
              ...
              return (
                <div key={t.id} className="flex items-center justify-between p-3 rounded-xl bg-[var(--vuno-bg)]">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button
                      onClick={() => { confirmTransfer(t.id).catch(() => {}); }}
                ...
                {filtered.filter(p => p.warehouseStock > 0).map(item => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button
                        onClick={() => { void handleQuickTransfer(item, 1); }}
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  <StockStatusBadge isLow={isLow} className="px-2 py-1 flex-shrink-0" />
                </div>
            ...
            return (
              ...
              >
                ...
                <span className="w-20 flex-shrink-0">
                  <StockStatusBadge isLow={isLow} className="px-2 py-1" />
                </span>
