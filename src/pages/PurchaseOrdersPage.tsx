...
const purchaseOrderStatusStyles: Record<string, { badgeClass: string; label: string }> = {
  pending: { badgeClass: 'bg-amber-100 text-amber-600', label: 'معلقة' },
  approved: { badgeClass: 'bg-blue-100 text-blue-600', label: 'معتمدة' },
  received: { badgeClass: 'bg-emerald-100 text-emerald-600', label: 'مستلمة' },
};

function PurchaseOrderStatusBadge({ status }: { status: string }) {
  const { badgeClass, label } = purchaseOrderStatusStyles[status] ?? purchaseOrderStatusStyles.pending;
  return <span className={`px-2 py-1 rounded-lg text-xs font-medium ${badgeClass}`}>{label}</span>;
}
...
export default function PurchaseOrdersPage() {
  ...
  // Orders + kanban come from the clean PurchaseOrderService via usePurchaseOrders().
  const { orders: ordersList, kanbanOrders, updateKanbanOrderStatus } = usePurchaseOrders();
  ...
  const handleAdvance = (id: string) => {
    const current = kanbanOrders.find(o => o.id === id);
    if (!current) return;
    const idx = statusOrder.indexOf(current.status);
    if (idx < 0 || idx >= statusOrder.length - 1) return;
    void updateKanbanOrderStatus(id, statusOrder[idx + 1]);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <StatsRow
        maxCols={4}
        items={[
          { label: 'إجمالي الطلبات', value: ordersList.length.toString(), icon: PurchaseOrdersIcon, color: 'bg-[var(--vuno-surface-pearl)] text-[var(--vuno-primary)]' },
          { label: 'معلقة', value: ordersList.filter(o => o.status === 'pending').length.toString(), icon: PurchaseOrdersIcon, color: 'bg-amber-50 text-amber-500' },
          { label: 'معتمدة', value: ordersList.filter(o => o.status === 'approved').length.toString(), icon: PurchaseOrdersIcon, color: 'bg-emerald-50 text-emerald-500' },
          { label: 'إجمالي القيمة', value: `${(ordersList.reduce((s, o) => s + o.total, 0) / 1000).toFixed(1)}K EGP`, icon: PurchaseOrdersIcon, color: 'bg-[var(--vuno-surface-pearl)] text-[var(--vuno-primary)]' },
        ]}
      />
      ...
      <div className="space-y-3">
        <SearchBar
          ...
          qrValue={`vuno:purchase-orders:${ordersList.length}`}
          ...
          <div className="hidden md:block card-vuno overflow-hidden">
            ...
            {filtered.map((order, i) => (
              <motion.div key={order.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(i * 0.05, 0.3) }} className="list-row">
                ...
                <span className="w-24 flex-shrink-0"><PurchaseOrderStatusBadge status={order.status} /></span>
