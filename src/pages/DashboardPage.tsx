...
export default function DashboardPage() {
  ...
  // ── Data comes from the clean service layer (never services/mock directly) ──
  const { getSalesData, getTopProducts, getHeatmapCells, getTopPerformers, getBottomPerformers } = useReporting();
  ...
  const heatmapCells = useMemo(() => getHeatmapCells(), [getHeatmapCells]);
  const topPerformers = useMemo(() => getTopPerformers(), [getTopPerformers]);
  const bottomPerformers = useMemo(() => getBottomPerformers(), [getBottomPerformers]);
  const recentInvoices = useMemo(() => invoices.slice(0, 5), [invoices]);
  const totalPurchases = useMemo(() => orders.reduce((sum, o) => sum + o.total, 0), [orders]);
      ...
      <div className="w-full lg:max-w-6xl lg:mx-auto">
        ...
        >
          ...
          {flashSales.some(s => s.active) && (
            <div className="border-t border-[var(--vuno-border-light)] bg-[var(--vuno-bg)] px-5 sm:px-8 py-5">
              <FlashSales sales={flashSales.filter(s => s.active)} variant="full" />
            </div>
          )}
