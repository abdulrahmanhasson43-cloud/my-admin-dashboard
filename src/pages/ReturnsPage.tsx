...
import {
  ...
  type ReturnRequest, type ReturnType, type ReturnReason,
} from '@/types/return';
...
/* ─────────────────────────────────────────────────────────────────────
   CreateReturnModal — gathers input only. Building the actual
   ReturnRequest (id, status, refundAmount, date) is ReturnService's job,
   not the UI's — this modal just calls onCreate with the raw input.
   ───────────────────────────────────────────────────────────────────── */
function CreateReturnModal({
  ...
  type: ReturnType;
  onClose: () => void;
  onCreate: (input: CreateReturnInput) => void;
}) {
  const { invoices } = useInvoices();
  const [partyName, setPartyName] = useState('');
  const [partyPhone, setPartyPhone] = useState('');
  const [originalInvoiceId, setOriginalInvoiceId] = useState(invoices[0]?.id ?? '');
  ...
  const invoiceProducts = useMemo(() => {
    const inv = invoices.find(i => i.id === originalInvoiceId);
    if (!inv) return [];
  ...
  // Preview only — the authoritative refund total is (re)computed by
  // ReturnService when the return is actually created.
...
export default function ReturnsPage() {
  ...
  const { returns, isLoading, error, refetch, advanceReturnStatus, createReturn } = useReturns();
  ...
  const advanceStatus = async (id: string) => {
    try {
      const updated = await advanceReturnStatus(id);
      toast.success(`تم تحديث حالة المرتجع إلى ${getReturnStatusMeta(updated.status).label}`);
    } catch {
      // The hook has already recorded the error in `error` — the banner shows it.
    }
  };
