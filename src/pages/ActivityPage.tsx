...
const activityTypeConfig: Record<
  ActivityEntry['type'],
  { icon: React.FC<{ className?: string; size?: number }>; color: string; label: string }
> = {
  sale: { icon: ReceiptIcon, color: 'var(--vuno-success)', label: 'مبيعة' },
  product: { icon: ProductsIcon, color: 'var(--vuno-primary)', label: 'منتج' },
  stock: { icon: InventoryIcon, color: 'var(--vuno-warning)', label: 'مخزون' },
  supplier: { icon: SuppliersIcon, color: 'var(--vuno-primary)', label: 'مورد' },
  shift: { icon: ArchiveIcon, color: 'var(--vuno-warning)', label: 'وردية' },
  settings: { icon: SettingsIcon, color: 'var(--vuno-text-muted)', label: 'إعدادات' },
};
        ...
        <div className="relative">
          ...
          <div className="space-y-3">
            {activities.map((activity, i) => {
              const { icon: Icon, color, label } = activityTypeConfig[activity.type];
              return (
                <motion.div
                  key={activity.id}
