...
// لون موحّد واحد لجميع الفئات (بدلًا من الألوان العشوائية)
...
export default function CategoriesPage() {
  ...
  const addCategory = async () => {
    ...
    if (editingId) {
      await updateCategory(editingId, { name: newName.trim() });
    } else {
      await createCategory({ name: newName, color: UNIFIED_CATEGORY_COLOR });
    }
    resetForm();
  };

  const handleEdit = (id: string, name: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingId(id);
    setNewName(name);
    setShowForm(true);
  ...
  const handleDeleteProduct = async (productId: string, productName: string, e: React.MouseEvent) => {
    ...
    if (window.confirm(`متأكد إنك عايز تحذف "${productName}" من الفئة؟`)) {
      try {
        ...
        // The context has already surfaced the error (banner + toast).
      }
    }
  };
  ...
  // عرض تفاصيل الفئة المختارة (محتوياتها)
              ...
              {selectedProducts.map((p, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <button
                      onClick={(e) => { void handleDeleteProduct(p.id, p.name, e); }}
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div />
        <button
          onClick={() => { setShowForm(!showForm); setEditingId(null); setNewName(''); }}
          className="px-5 py-2.5 rounded-full text-white font-medium flex items-center gap-2 btn-primary-pill hover:opacity-90 transition-opacity"
        >
          <PlusIcon size={16} />
          فئة جديدة
      ...
      {showForm && (
        ...
        >
          <h3 className="font-bold text-[var(--vuno-text)] mb-4">{editingId ? 'تعديل الفئة' : 'إضافة فئة جديدة'}</h3>
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={resetForm} className="px-4 py-3 rounded-full border border-[var(--vuno-border)] text-[var(--vuno-text-secondary)] hover:bg-[var(--vuno-bg)] transition-colors">
                إلغاء
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {categories.map((cat, i) => (
          <motion.div
            ...
            tabIndex={0}
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') setSelectedCat(cat.name);
            }}
            className="card-vuno p-6 hover:border-[var(--vuno-primary)] transition-all group text-right flex flex-col active-scale cursor-pointer"
          >
            <div className="flex items-start justify-between mb-3">
              ...
              {/* أزرار التعديل والحذف — مرئية دائمًا على الموبايل، تظهر عند المرور على الديسكتوب */}
              <div className="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                <button
                  onClick={(e) => handleEdit(cat.id, cat.name, e)}
            ...
            <div className="mt-3 h-1.5 rounded-full bg-[var(--vuno-bg)] overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(cat.productCount, 100)}%` }}
      ...
      <p className="text-xs text-[var(--vuno-text-muted)] text-center">
        اضغط على أي فئة لعرض محتوياتها ومنتجاتها
      </p>
    </div>
  );
}
