          ...
          {brandColorPresets.map(preset => {
            ...
            return (
              ...
              >
                ...
                {isSelected && (
                  <CheckIcon size={14} className="absolute top-1 left-1" style={{ color: preset.primary }} />
                )}
...
export default function ProfilePage() {
  const {
    ...
    updateBranding: persistBranding,
  } = useProfile();

  // Local, uncommitted edits layered on top of the service data. Deriving the
  // effective value (instead of syncing it into state through an effect) keeps
  // the component free of cascading renders.
  const [profileEdits, setProfileEdits] = useState<Partial<UserProfile>>({});
  const [brandingEdits, setBrandingEdits] = useState<Partial<StoreBranding>>({});

  const profile: UserProfile = { ...(loadedProfile ?? defaultUserProfile), ...profileEdits };
  const branding: StoreBranding = { ...(loadedBranding ?? defaultStoreBranding), ...brandingEdits };

  const updateProfile = (patch: Partial<UserProfile>) => setProfileEdits(prev => ({ ...prev, ...patch }));
  const updateBranding = (patch: Partial<StoreBranding>) => setBrandingEdits(prev => ({ ...prev, ...patch }));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-10">
      ...
      >
        ...
        <SubscriptionCard subscription={subscription ?? defaultSubscription} />
