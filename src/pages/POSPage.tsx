...
export default function POSPage() {
  ...
  const handleCheckout = async () => {
    ...
    // Persist the sale FIRST and only clear the cart once the mutation is
    // confirmed. If it fails, the context has already surfaced the error
    // (banner + toast) and we keep the cart intact so the cashier can retry —
    // this is what prevents the "double sale" scenario.
    ...
    // The sale is safely persisted — now run the success feedback and reset.
    setShowSuccess(true);
    // الفكرة #4: تسجيل البيع في الوردية الحالية
    recordSale(total);
    // الفكرة #3: تحديث تقدم الهدف البيعي
    addAchieved(total);
