...
export default function AnalyticsPage() {
  // All analytics data comes from the clean Reporting service (never the mock layer).
  const {
    getHeatmapCells, getTopPerformers, getBottomPerformers,
    getPeriodData, getAvailablePeriods, getPriceSuppliers,
    getPriceHistory, getPriceHistoryProducts,
  } = useReporting();
  ...
  const periodData = useMemo(() => getPeriodData(), [getPeriodData]);
  const availablePeriods = useMemo(() => getAvailablePeriods(), [getAvailablePeriods]);
  const priceSuppliers = useMemo(() => getPriceSuppliers(), [getPriceSuppliers]);

  // Build heatmap data once (extracts insights from the cells)
  ...
  const priceProducts = useMemo(() => getPriceHistoryProducts(), [getPriceHistoryProducts]);
  const [selectedProduct, setSelectedProduct] = useState(priceProducts[0] || '');
  const pricePoints = useMemo(
    () => getPriceHistory(selectedProduct),
    [getPriceHistory, selectedProduct],
  );
