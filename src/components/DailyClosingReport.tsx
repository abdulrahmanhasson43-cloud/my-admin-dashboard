...
function buildHourlyData(invoices: Invoice[]) {
...
function buildPaymentBreakdown(invoices: Invoice[]) {
...
export default function DailyClosingReport() {
  ...
  // Clean data sources — never the mock layer.
  const { invoices } = useInvoices();
  const { expenses } = useExpenses();
  const { getTopProducts } = useReporting();
  ...
  const paymentBreakdown = useMemo(() => buildPaymentBreakdown(invoices), [invoices]);
  const hourlyData = useMemo(() => buildHourlyData(invoices), [invoices]);
  ...
  }, [todayStr, topProducts, totalSales, invoiceCount, customerCount, totalExpenses, lowStockItems.length, openingCash, expectedCash, countedCash, variance]);
