...
import type { LoyaltySummary, RewardOption, RewardType } from '@/types';
import { loyaltyLevels, rewardOptions } from '@/types';
import {
  CoinsIcon,
  StarIcon,
  TrendingUpIcon,
...
/** أيقونة كل نوع مكافأة — خريطة منفصلة عن بيانات rewardOptions نفسها (نفس
 * نمط PAYMENT_ICONS في OrdersPage)، حتى يظل types/loyalty.ts خاليًا من أي
 * اعتماد على مكونات الواجهة. */
const REWARD_ICONS: Record<RewardType, typeof CoinsIcon> = {
  discount_50: CoinsIcon,
  discount_120: CashIcon,
  free_product: GiftIcon,
};
      ...
      <div className={cn('rounded-2xl p-4 mb-4', level.bgColor)}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <MedalIcon size={22} className={level.color} />
            <div>
        ...
        {nextLevel ? (
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[10px] text-[var(--vuno-text-secondary)] inline-flex items-center gap-1">
                تحتاج {pointsToNext.toLocaleString()} نقطة للمستوى {nextLevel.label}
                <MedalIcon size={11} className={nextLevel.color} />
              </span>
      ...
      <div className="flex items-center justify-between gap-1 mb-4">
        {loyaltyLevels.map((lvl) => {
          ...
          return (
            ...
            >
              <MedalIcon size={16} className={isPassed ? lvl.color : 'text-[var(--vuno-text-muted)]'} />
...
function RewardCard({
  ...
  return (
    <div className={cn(
      ...
      <div className="w-9 h-9 rounded-lg bg-[var(--vuno-surface-pearl)] flex items-center justify-center flex-shrink-0 text-[var(--vuno-primary)]">
        <RewardIcon size={18} />
      </div>
