...
import type { CustomerCLV as CLVData, CustomerTier } from '@/types';
...
/** أيقونة كل مستوى عميل — نفس نمط PAYMENT_ICONS في OrdersPage: خريطة أيقونات
 * منفصلة عن بيانات tierMeta نفسها، حتى تظل types/loyalty.ts خالية من أي
 * اعتماد على مكونات الواجهة (Dependency Rule). */
const TIER_ICONS: Record<CustomerTier, typeof StarIcon> = {
  vip: StarIcon,
  excellent: GemIcon,
  regular: UserIcon,
  dormant: MoonIcon,
...
export default function CustomerCLV({ client, activities = [], variant = 'full' }: CustomerCLVProps) {
  ...
  const TierIcon = TIER_ICONS[clv.tier];
